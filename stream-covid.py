...
st.markdown('This app gives you the realtime impact analysis of Covid-19 in India.')
...
if india_checkbox:
    ...
    if not st.checkbox('Hide', False, key=1):
        india_total_chart = px.bar(india_total, x='Status', y='Number of cases', color='Status', width=700)
    ...
    datatable = get_table()
    st.markdown("### Covid-19 cases in India : State wise")
    st.write(datatable)
...
if state_checkbox:
    ...
    if not st.checkbox('Hide', False, key=4):
        state_total = px.bar(state_wise_total, x='Status', y='Number of cases', labels={'Number of cases':'Number of cases in %s' % (select)}, color='Status', width=700)
    ...
    if not st.checkbox('Hide', False, key=6):
        ...
        if(select_visual == 'Line chart'):
            ...
            facet_row='Status', height=600, width=800, labels={states[select]:"cases"})
